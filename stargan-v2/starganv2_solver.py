...
# 1. 현재 실행 중인 파일(starganv2_solver.py)의 절대 경로를 가져옵니다.
current_dir = os.path.dirname(os.path.abspath(__file__))

# 2. 현재 경로에서 한 단계 위(..)인 프로젝트 루트 경로(IT-RAP)를 계산합니다.
# os.path를 사용하면 Windows 환경의 백슬래시(\) 경로 문제도 자동으로 안전하게 처리됩니다.
root_dir = os.path.abspath(os.path.join(current_dir, '..'))

# 3. 파이썬이 모듈을 탐색할 경로(sys.path)의 가장 앞(0번 인덱스)에 루트 경로를 추가합니다.
...
class SolverRainbow(object):
    def __init__(self, dataset_loader, config):
        ...
        # StarGAN-v2용 전처리 파이프라인
        self.stargan_transform = transforms.Compose([
            transforms.Resize([256, 256]),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
        ])

        # 공통으로 사용할 5개 레퍼런스 이미지 경로 및 도메인(0:여성, 1:남성) 설정
        # TODO: 실제 프로젝트 환경에 맞게 경로와 도메인을 수정!
        self.reference_images = [
            {'path': 'C:\\Users\\Bandi\\Desktop\\Fork\\IT-RAP\\stargan-v2\\assets\\celeba_hq\\ref\\female\\ref_blond_female.jpg', 'domain': 0},
            {'path': 'C:\\Users\\Bandi\\Desktop\\Fork\\IT-RAP\\stargan-v2\\assets\\celeba_hq\\ref\\female\\ref_old_female.jpg', 'domain': 0},
            {'path': 'C:\\Users\\Bandi\\Desktop\\Fork\\IT-RAP\\stargan-v2\\assets\\celeba_hq\\ref\\male\\ref_bald_male.jpg', 'domain': 1},
            {'path': 'C:\\Users\\Bandi\\Desktop\\Fork\\IT-RAP\\stargan-v2\\assets\\celeba_hq\\ref\\male\\ref_young_male.jpg', 'domain': 1},
            {'path': 'C:\\Users\\Bandi\\Desktop\\Fork\\IT-RAP\\stargan-v2\\assets\\celeba_hq\\ref\\male\\ref_black_male.jpg', 'domain': 1}
        ]
        self.common_style_codes = [] # 추출된 스타일 코드를 저장할 리스트
    ...
    def inference_rainbow_dqn(self, data_loader, result_dir):
        ...
        self.attack_func = starganv2_attacks.AttackFunction(config=self.config, model=self.nets_ema.generator, device=self.device)
        ...
        for infer_img_idx, (x_real, c_org, filename) in enumerate(data_loader):
            ...
            for idx, c_trg in enumerate(c_trg_list):
                ...
                for step in range(self.max_steps_per_episode):
                    ...
                    with torch.no_grad():
                        original_gen_image = self.nets_ema.generator(x_real, c_trg)
                        perturbed_gen_image = self.nets_ema.generator(perturbed_image, c_trg)
                ...
                with torch.no_grad():
                    ...
                    original_gen_image = self.nets_ema.generator(x_real, c_trg)
                    perturbed_gen_image_orig = self.nets_ema.generator(perturbed_image, c_trg)
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_jpeg = self.nets_ema.generator(x_adv_jpeg, c_trg)
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_opencv = self.nets_ema.generator(x_adv_denoise_opencv, c_trg)
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_median = self.nets_ema.generator(x_adv_median, c_trg)
                ...
                with torch.no_grad():
                    ...
                    original_gen_image_padding = self.nets_ema.generator(x_real_padding, c_trg)
                    perturbed_gen_image_padding = self.nets_ema.generator(x_adv_padding, c_trg)
                ...
                with torch.no_grad():
                    ...
                    original_gen_image_transforms = self.nets_ema.generator(x_real_transforms, c_trg)
                    perturbed_gen_image_transforms = self.nets_ema.generator(x_adv_transforms, c_trg)
    ...
    def build_model(self):
        """StarGAN-v2 모델을 로드"""
        print("StarGAN-v2 모델을 빌드하고 사전 학습된 가중치를 로드합니다...")
        args = Munch({
            'latent_dim': 16,
            'hidden_dim': 512,
            'style_dim': 64,
            ...
            'w_hpf': 1,
            'img_size': 256,
            'wing_path': 'C:\\Users\\Bandi\\Desktop\\Fork\\IT-RAP\\stargan-v2\\expr\\checkpoints\\wing.ckpt' # 실제 경로로 맞춤 조정
        })

        nets, nets_ema = build_model(args)
        
        checkpoint_dir = 'C:\\Users\\Bandi\\Desktop\\Fork\\IT-RAP\\stargan-v2\\expr\\checkpoints\\celeba_hq'
        ckpt_template = os.path.join(checkpoint_dir, '{}_nets_ema.ckpt')
        ckptio = CheckpointIO(ckpt_template, data_parallel=True, **nets_ema)
        ckptio.load(100000)
    ...
    def extract_common_style_codes(self):
        """5개의 지정된 레퍼런스 이미지로부터 스타일 코드를 사전에 1회만 추출합니다."""
        print("공통 레퍼런스 이미지 5개에서 스타일 코드를 추출합니다...")
        self.common_style_codes = []
        
        for ref in self.reference_images:
            img = self.stargan_transform(Image.open(ref['path']).convert('RGB')).unsqueeze(0).to(self.device)
            y_trg = torch.tensor([ref['domain']]).to(self.device)
            
            with torch.no_grad():
                style_code = self.nets_ema.style_encoder(img, y_trg)
                self.common_style_codes.append(style_code)
        
        print(f"총 {len(self.common_style_codes)}개의 스타일 코드 추출 완료!")
    ...
    """
    def calculate_reward(self, original_gen_image, perturbed_gen_image, x_real, perturbed_image, style_code):
        transformed_x_real, transformed_perturbed_image = apply_random_transform(x_real, perturbed_image)
        transformed_original = self.nets_ema.generator(transformed_x_real, style_code)
        transformed_perturbed = self.nets_ema.generator(transformed_perturbed_image, style_code)

        # TODO: LPIPS 계산 직전에 텐서 값을 -1 ~ 1 사이로 강제 고정(clamping)합니다.
        transformed_original = torch.clamp(transformed_original, min=-1.0, max=1.0)
        transformed_perturbed = torch.clamp(transformed_perturbed, min=-1.0, max=1.0)
    ...
    def train_attack(self):
        ...
        # 1. 훈련 시작 전 스타일 코드 5개를 미리 추출합니다.
        self.extract_common_style_codes()
        ...
        for test_img_idx, (x_real, c_org, filename) in enumerate(data_loader):
            ...
            attack_func = starganv2_attacks.AttackFunction(config=self.config, model=self.nets_ema.generator, device=self.device)
            ...
            for idx, style_code in enumerate(self.common_style_codes):
                print("=" * 100)
                print(f"Style Code index : {idx + 1}")
                ...
                with torch.no_grad():
                    original_gen_image = self.nets_ema.generator(x_real, style_code)
                    perturbed_gen_image = self.nets_ema.generator(perturbed_image, style_code)
                ...
                for step in range(self.max_steps_per_episode):
                    ...
                    if action == 0:
                        perturbed_image, _ = attack_func.PGD(perturbed_image, original_gen_image, style_code)

                    elif action == 1:
                        perturbed_image, _ = attack_func.perturb_frequency_domain(perturbed_image, original_gen_image, style_code, freq_band='LOW')

                    elif action == 2:
                        perturbed_image, _ = attack_func.perturb_frequency_domain(perturbed_image, original_gen_image, style_code, freq_band='MID')

                    elif action == 3:
                        perturbed_image, _ = attack_func.perturb_frequency_domain(perturbed_image, original_gen_image, style_code, freq_band='HIGH')
                    ...
                    reward, defense_l1_loss, defense_l2_loss, defense_lpips, invisibility_ssim, invisibility_psnr, invisibility_lpips = self.calculate_reward(original_gen_image, perturbed_gen_image, x_real, perturbed_image, style_code)
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_orig = self.nets_ema.generator(perturbed_image, style_code)
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_jpeg = self.nets_ema.generator(x_adv_jpeg, style_code)
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_opencv = self.nets_ema.generator(x_adv_denoise_opencv, style_code)
                ...
                with torch.no_grad():
                    ...
                    perturbed_gen_image_median = self.nets_ema.generator(x_adv_median, style_code)
                ...
                with torch.no_grad():
                    ...
                    original_gen_image_padding = self.nets_ema.generator(x_real_padding, style_code)
                    perturbed_gen_image_padding = self.nets_ema.generator(x_adv_padding, style_code)
                ...
                with torch.no_grad():
                    ...
                    original_gen_image_transforms = self.nets_ema.generator(x_real_transforms, style_code)
                    perturbed_gen_image_transforms = self.nets_ema.generator(x_adv_transforms, style_code)
