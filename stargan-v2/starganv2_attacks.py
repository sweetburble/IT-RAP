...
class AttackFunction(object):
    def __init__(self, config, model, device=None, epsilon=0.05, a=0.01):
        self.model = model # model: StarGAN-v2의 nets_ema.generator
    ...
    """
    Role: Performs a basic I-FGSM/PGD attack.
    ...
    """
    def PGD(self, X_nat, y, style_code):
        ...
        for i in range(iter):
            ...
            output = self.model(X, style_code)
    ...
    def perturb_frequency_domain(self, X_nat, y, style_code, freq_band='ALL', iter=1):
        ...
        for i in range(iter):
            ...
            # StarGAN-v2 generator 호출 방식 및 단일 반환값 처리
            output = self.model(X, style_code)
