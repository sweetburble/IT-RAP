...
class SolverRainbow(object):
    ...
    # =========================================================================
    def _run_calibration_attgan(self, calib_loader, calib_image_num):
        """Run calibration to compute per-transform action rankings for AttGAN."""
        print(f"[Greedy Calibration] Starting calibration on {calib_image_num} images...")
        calib_attack_func = attgan_attacks.AttackFunction(
            config=self.config, model=attgan_model.G, device=self.device)
        ...
        for img_idx, (x_real, c_org, filename) in enumerate(calib_loader):
            ...
            for action in range(4):
                ...
                with torch.no_grad():
                    x_adv_jpeg = compress_jpeg(perturbed_image, quality=75)
                    gen_jpeg = attgan_model.G(x_adv_jpeg, c_trg, mode='enc-dec')
                    action_gains["jpeg"][action] += F.mse_loss(original_gen_image, gen_jpeg).item()
                    action_counts["jpeg"][action] += 1

                    x_adv_opencv = denoise_opencv(perturbed_image)
                    gen_opencv = attgan_model.G(x_adv_opencv, c_trg, mode='enc-dec')
                    action_gains["opencv"][action] += F.mse_loss(original_gen_image, gen_opencv).item()
                    action_counts["opencv"][action] += 1

                    x_adv_median = denoise_scikit(perturbed_image)
                    gen_median = attgan_model.G(x_adv_median, c_trg, mode='enc-dec')
                    action_gains["median"][action] += F.mse_loss(original_gen_image, gen_median).item()
                    action_counts["median"][action] += 1

                    x_real_pad, x_adv_pad = random_resize_padding(x_real, perturbed_image)
                    orig_gen_pad = attgan_model.G(x_real_pad, c_trg, mode='enc-dec')
                    gen_pad = attgan_model.G(x_adv_pad, c_trg, mode='enc-dec')
                    action_gains["padding"][action] += F.mse_loss(orig_gen_pad, gen_pad).item()
                    action_counts["padding"][action] += 1

                    x_real_aff, x_adv_aff = random_image_transforms(x_real, perturbed_image)
                    orig_gen_aff = attgan_model.G(x_real_aff, c_trg, mode='enc-dec')
                    gen_aff = attgan_model.G(x_adv_aff, c_trg, mode='enc-dec')
                    action_gains["affine"][action] += F.mse_loss(orig_gen_aff, gen_aff).item()
    ...
    def inference_greedy_policy(self, data_loader, result_dir, calib_loader, calib_image_num=50):
        """Inference using greedy action selection based on calibration-set rankings (AttGAN)."""
        os.makedirs(result_dir, exist_ok=True)
        attack_func = attgan_attacks.AttackFunction(config=self.config, model=attgan_model.G, device=self.device)

        rankings = self._run_calibration_attgan(calib_loader, calib_image_num)
        ...
        for infer_img_idx, (x_real, c_org, filename) in enumerate(data_loader):
            ...
            for idx, c_trg in enumerate(c_trg_list):
                ...
                for step in range(self.max_steps_per_episode):
                    ...
                    action = rankings[transform_name][0]
    ...
    def get_state(self, perturbed_image, perturbed_gen_image, force_extract=False):
        ...
        return combined_features, False
